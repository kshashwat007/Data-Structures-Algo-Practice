...
def flood_fill(img, row, col, val):
    ...
    while queue:
        ...
        visited.add((row, col))
        img[row][col] = val
        for row, col in neighbour(img, row, col, start):
            if (row, col) not in visited:
                queue.append((row, col))
    return img


def neighbour(img, row, col, start):
    visit = [(row-1, col), (row+1, col), (row, col-1), (row, col+1)]
    return [(row, col) for row, col in visit if isValid(img, row, col) and img[row][col] == start]
